...
def create_new_database():
    ...
    new_db_name = filedialog.asksaveasfilename(defaultextension=".db", filetypes=[("SQLite Database Files", "*.db")])
    ...
    if new_db_name:
        ...
        new_db_filename = os.path.basename(new_db_name)
...
def update_email_domain(csv_filename):
    try:
        with open(csv_filename, 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                text_to_append = row['Status']  # Read the text from the CSV
                if text_to_append:
                    first_name = fn_entry.get()
                    last_name = ln_entry.get()
                    updated_email = f"{first_name.lower()}.{last_name.lower()}{text_to_append}"
                    em_entry.delete(0, END)
...
ad_menu.add_command(label="Extract OU to CSV", command=get_OU_and_extract)
ad_menu.add_separator()
...
import_csv_button.grid(row=3, column=0, padx=10, pady=16, sticky='w')
...
create_sam_at_domain_button.grid(row=1, column=1, padx=10, pady=10, sticky='w')

create_ou_button = ttk.Button(mgmtou_frame, text="Create Organisation Unit Path", style="Custom.TButton", command=create_ou_csv)
create_ou_button.grid(row=1, column=0, padx=10, pady=15, sticky='w')
